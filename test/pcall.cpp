...
BOOST_AUTO_TEST_CASE(lua_pcall_0_results)
{
	test::test_with_environment([](lua::stack &s, test::resource bound)
	{
		...
		lua::stack_array results = lua::pcall(*s.state(), 0, static_cast<lua_Integer>(0));
...
BOOST_AUTO_TEST_CASE(lua_pcall_1_result)
{
	test::test_with_environment([](lua::stack &s, test::resource bound)
	{
		...
		lua::stack_array results = lua::pcall(*s.state(), 0, static_cast<lua_Integer>(1));
...
BOOST_AUTO_TEST_CASE(lua_pcall_2_results)
{
	test::test_with_environment([](lua::stack &s, test::resource bound)
	{
		...
		lua::stack_array results = lua::pcall(*s.state(), 0, static_cast<lua_Integer>(2));
...
BOOST_AUTO_TEST_CASE(lua_pcall_2_arguments)
{
	test::test_with_environment([](lua::stack &s, test::resource bound)
	{
		...
		script.release();
		auto function = lua::pcall(*s.state(), 0, static_cast<lua_Integer>(1));
